...
/**
 * Strings containing the axis and button functions
 */
extern const char* ButtonStrings[];
extern const char* AxisStrings[];
extern const char* AxisStringsGUI[];
 ...
 */
extern const char* InputMethodStrings[];
...
class T_TX_Mixer
{
  public:
    ...
    float nrate_val[T_AxisMapper::NUM_AXISFUNCS];
...
class T_Calibration
{
  public:
   ...
   /**
    * Set the scaling for one axis.
    */
    void setScaling(int axis, float val) {scale[axis] = val;};
    
   /**
    * Set the offset for one axis.
    */
    void setOffset(int axis, float val) {offset[axis] = val;};

   /**
    * Get the scaling for one axis.
    */
    float getScaling(int axis) {return scale[axis];};
    
   /**
    * Get the offset for one axis.
    */
    float getOffset(int axis) {return offset[axis];};

    
  private:
    std::string child_in_cfg;
    float scale[TX_MAXAXIS];
    float offset[TX_MAXAXIS];
    T_TX_Interface *iface;    ///< pointer to 'parent' interface
};
