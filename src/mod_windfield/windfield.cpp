...
void draw_wind(double direction_face)
{
  ...
  // Hintergrund
  glColor3f (0, 0, 0);
  glTranslatef(window_xsize-r-h/2, r+h/2, 0);
  gluDisk(therm_quadric, 0, h/2, 32, 1);
  glTranslatef(-(window_xsize-r-h/2),-(r+h/2),0.1);

  // Anzeiger
  glColor3f (0, 1, 0.);
  glBegin(GL_LINE_STRIP);
  glVertex2i(window_xsize - r - h/2 - dxC, r+h/2 - dyC);
  glVertex2i(window_xsize - r - h/2 - dxA, r+h/2 - dyA);
  glVertex2i(window_xsize - r - h/2 - dxB, r+h/2 - dyB);
  glVertex2i(window_xsize - r - h/2 - dxC, r+h/2 - dyC);
  glEnd();
