...
void T_Wind::read(SimpleXMLTransfer* cfgfile, T_Config* cfg)
{
  ...
  try
  {
    ...
    flDirection = el->getDouble("direction");
...
void T_Config::createDefaultConfig(SimpleXMLTransfer*& acfgfile)
{
  ...
  tex = new SimpleXMLTransfer();
  tex->setName("thermal");
  tex->addAttribute("name_en",      "F3F (no thermals)");
...
void T_Config::read(SimpleXMLTransfer* cfgfile)
{
  ...
  setLocation(cfgfile->getString("location.name").c_str(), cfgfile, false);
...
{
  setLocation(locstr, cfgfile, true);
}
                           ...
                           bool                fReadAgain)
{
  ...
  printf("T_Config: Setting location to %s\n", tmp.c_str());
