...
void CImageLoaderTGA::LoadHeader() 
{
  ...
  nNotUsed = fread(&cGarbage, sizeof(unsigned char), 1, file);
  nNotUsed = fread(&cGarbage, sizeof(unsigned char), 1, file);

// type must be 2 or 3
  nNotUsed = fread(&type, sizeof(unsigned char), 1, file);

  nNotUsed = fread(&iGarbage, sizeof(short int), 1, file);
  nNotUsed = fread(&iGarbage, sizeof(short int), 1, file);
  nNotUsed = fread(&cGarbage, sizeof(unsigned char), 1, file);
  nNotUsed = fread(&iGarbage, sizeof(short int), 1, file);
  nNotUsed = fread(&iGarbage, sizeof(short int), 1, file);

  nNotUsed = fread(&width, sizeof(short int), 1, file);
  nNotUsed = fread(&height, sizeof(short int), 1, file);
  nNotUsed = fread(&pixelDepth, sizeof(unsigned char), 1, file);
...
void CImageLoaderTGA::LoadImageData()
{
  ...
  if (type & TGA_TYPE_RLE_COMPRESSION_BIT)
  {
    ...
    while (BytesDecoded < total)
    {
      ...
      nNotUsed = fread(&RLHeader, sizeof(unsigned char), 1, file);
      ...
      if (RLHeader & 0x80)
      {
        ...
        int i, k;
        nNotUsed = fread(pixel, sizeof(unsigned char), mode, file);
      ...
      else
      {
        ...
        nNotUsed = fread(&imageData[BytesDecoded], sizeof(unsigned char),
                         ChunkSize*mode, file);
  ...
  else
  {
    ...
    nNotUsed = fread(imageData,sizeof(unsigned char),total,file);
  }
                          ...
                          unsigned char *imageData) 
{
  ...
  // write the header
  nNotUsed = fwrite(&cGarbage, sizeof(unsigned char), 1, file);
  nNotUsed = fwrite(&cGarbage, sizeof(unsigned char), 1, file);

  nNotUsed = fwrite(&type, sizeof(unsigned char), 1, file);

  nNotUsed = fwrite(&iGarbage, sizeof(short int), 1, file);
  nNotUsed = fwrite(&iGarbage, sizeof(short int), 1, file);
  nNotUsed = fwrite(&cGarbage, sizeof(unsigned char), 1, file);
  nNotUsed = fwrite(&iGarbage, sizeof(short int), 1, file);
  nNotUsed = fwrite(&iGarbage, sizeof(short int), 1, file);

  nNotUsed = fwrite(&width, sizeof(short int), 1, file);
  nNotUsed = fwrite(&height, sizeof(short int), 1, file);
  nNotUsed = fwrite(&pixelDepth, sizeof(unsigned char), 1, file);
  ...
  // save the image data
  nNotUsed = fwrite(imageData, sizeof(unsigned char), width * height * mode, file);
  fclose(file);
  // release the memory
  free(imageData);
