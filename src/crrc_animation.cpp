 ...
 */
CRRCControlSurfaceAnimation::CRRCControlSurfaceAnimation(SimpleXMLTransfer *xml, TSimInputs *inputs)
 : CRRCAnimation(new ssgTransform()), fallback_data(0.0f)
{
  ...
  for (int i = 0; i < xml->getChildCount(); i++)
  {
    ...
    if (child->getName() == "hinge")
    {
      // found a <hinge> child
      sgVec3 pos;
      //~ pos[SG_X] = child->getDouble("x", 0.0f);
      //~ pos[SG_Y] = child->getDouble("y", 0.0f);
      //~ pos[SG_Z] = child->getDouble("z", 0.0f);
    ...
    else if (child->getName() == "control")
    {
      ...
      if (mapping == "ELEVATOR")
      {
        datasource.push_back(&(inputs->elevator));
        source_gain.push_back(gain * 2);
        num_controls++;
      }
      else if (mapping == "AILERON")
      {
        datasource.push_back(&(inputs->aileron));
        source_gain.push_back(gain * 2);
        num_controls++;
      }
      else if (mapping == "THROTTLE")
      {
        datasource.push_back(&(inputs->throttle));
        source_gain.push_back(gain * 2);
        num_controls++;
      }
      else if (mapping == "RUDDER")
      {
        datasource.push_back(&(inputs->rudder));
        source_gain.push_back(gain * 2);
        num_controls++;
      }
      else if (mapping == "FLAP")
      {
        datasource.push_back(&(inputs->flap));
        source_gain.push_back(gain * 2);
        num_controls++;
      }
      else if (mapping == "SPOILER")
      {
        datasource.push_back(&(inputs->spoiler));
        source_gain.push_back(gain * 2);
        num_controls++;
      }
      else if (mapping == "RETRACT")
      {
        datasource.push_back(&(inputs->retract));
        source_gain.push_back(gain * 2);
        num_controls++;
      }
      else if (mapping == "PITCH")
      {
        datasource.push_back(&(inputs->pitch));
        source_gain.push_back(gain * 2);
        num_controls++;
      }
      else
      {
        fprintf(stderr, "ControlSurfaceAnimation: ignoring <control> tag without mapping.\n");
      }
  ...
  if (num_controls < 1)
  {
    fprintf(stderr, "ControlSurfaceAnimation: found animation without proper <control> tag. Animation disabled.\n");
    failed = true;
  }

  if (num_hinges < 2)
  {
    fprintf(stderr, "ControlSurfaceAnimation: Must specify exactly two hinges!\n");
    failed = true;
  }
  else
  {
    ...
    {
      fprintf(stderr, "ControlSurfaceAnimation: Must specify exactly two hinges!\n");
      fprintf(stderr, "ControlSurfaceAnimation: Ignoring excessive hinge tag(s).\n");
    }
    sgSubVec3(axis, hinge_2, hinge_1);
    if (sgLengthVec3(axis) < 0.001)
    {
      fprintf(stderr, "ControlSurfaceAnimation: Insufficient spacing between hinges!\n");
      failed = true;
    }
  }
  ...
  if (failed)
  {
    fprintf(stderr, "ControlSurfaceAnimation: Animation setup failed.\n");
