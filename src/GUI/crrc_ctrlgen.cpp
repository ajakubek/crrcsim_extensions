...
// Order according to values of MOUSE and KEYBOARD!
static const char *zoomCtrl[]    = {"Keyboard", "Mouse", NULL};
            ...
            : CRRCDialog(0, 0, CRRC_DIALOG_OK)
{
  ...
  button_calibrate->setLegend("Configure...");
  ...
  button_joybuttons->setLegend("Buttons...");
  ...
  button_device->setLegend("Input Method...");
  ...
  combo_zoom->setLabel("Zoom Control");
  combo_zoom->setCurrentItem(Global::inputDev.zoom_control-1);
  combo_zoom->reveal();
  combo_zoom->activate();
...
{
  inputMethodString = "Input Method: ";
  inputMethodString += InputMethodStrings[Global::TXInterface->inputMethod()];
...
void CGUICtrlGeneralCallback(puObject *obj)
{
  ...
  {    
    ...
    Global::inputDev.zoom_control = 1+ dlg->combo_zoom->getCurrentItem();
  }  
  puDeleteObject(obj);
}
...
static void CGUIMixerButtonCallback(puObject *obj)
{
  if (Global::TXInterface->usesMixer())
  {
    new CGUIInputScaleDialog(Global::TXInterface);
  }
}
