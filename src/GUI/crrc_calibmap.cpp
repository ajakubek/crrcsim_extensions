 ...
 *
 * Copyright (C) 2005, 2008, 2009 Jan Reucker (original author)
  ...
  : CRRCDialog(0, 0, CRRC_DIALOG_OK), state(0), numaxis(0)
{
  ...
  for (int i = 0; i < NUM_FUNCTIONS; i++)
  {
    ...
    axismapping[i]->setLabel(AxisStringsGUI[i+1]);
  ...
  combo_radiotype->setLabel("Radio type");
  ...
  for (int i = 0; i < TX_MAXAXIS; i++)
  {
    axis[i] = new puSlider( i*DLG_DEF_SPACE + i * SLIDER_W,
  ...
  butCalibrate->setLegend("Calibrate");
  ...
  // The usual stuff... (resizing, centering)
  close();
  setSize(LABEL_W + (TX_MAXAXIS+2)*DLG_DEF_SPACE + DLG_CHECK_W + INPUT_W + 2*HORIZ_SPACER + TX_MAXAXIS * SLIDER_W,
...
void CGUICalibMapDialog::update()
{
  ...
  for (int i = 0; i < numaxis; i++)
  {
    ...
    if (Global::TXInterface->usesCalibration())
    {
      ...
      {
        case 1:
          // recalculate scale and offset if in calibration state 1
          if (maxval[i] != minval[i])
          {
            Global::TXInterface->calib->setScaling(i, (1.0 / (maxval[i] - minval[i])));
          }
          else
          {
            Global::TXInterface->calib->setScaling(i, 1.0);
          }
          Global::TXInterface->calib->setOffset(i, 
            Global::TXInterface->calib->getScaling(i) * (maxval[i] + minval[i]) / 2.0);
          break;
          
        case 2:
          // adapt offset in state 2
          error = Global::TXInterface->calib->calibrate(i, f);
          Global::TXInterface->calib->setOffset(i, 
            Global::TXInterface->calib->getOffset(i) - error);
          break;
...
static void CGUICalibMapCallback(puObject *obj)
{
  ...
  if (dlg->state == 0)
  {
    ...
    {
      // save mapping
      int rtype = dlg->combo_radiotype->getCurrentItem();
      if (rtype == T_AxisMapper::CUSTOM)
      {
        for (int i = 1; i <= NUM_FUNCTIONS; i++)
        {
          int axis = dlg->axismapping[(i-1)]->getCurrentItem();
          if (axis < dlg->numaxis)
          {
          ...
          if (dlg->invert[(i-1)]->getIntegerValue() == 1)
          {
...
static void CGUICalibButtonCallback(puObject *obj)
{
  ...
  switch (state)
  {
    default:
    case 0:
      dlg->butCalibrate->setLegend("Calibrate");
      dlg->butCalibrate->setLabel("");
      dlg->unlock();
      break;
    
    case 1:
      dlg->butCalibrate->setLegend("Next");
      dlg->butCalibrate->setLabel("Move controls to all extents!");
      dlg->lock();
      break;
    
    case 2:
      dlg->butCalibrate->setLegend("Finish");
      dlg->butCalibrate->setLabel("Now center all controls!");
      break;
  }
