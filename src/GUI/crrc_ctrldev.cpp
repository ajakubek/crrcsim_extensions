...
static const char *inputMethods[]    = {InputMethodStrings[0], InputMethodStrings[1],
                                        InputMethodStrings[2], InputMethodStrings[3],
...
: CRRCDialog(0, 0, CRRC_DIALOG_OK)
{
  combo_inputMethod = new puaComboBox(LABEL_W + DLG_DEF_SPACE, 
                                      ...
                                      (char **)inputMethods, false);
  combo_inputMethod->setLabelPlace(PUPLACE_LOWER_LEFT);
  combo_inputMethod->setLabel("Input Method");
  ...
  combo_device = new puaComboBox( LABEL_W + DLG_DEF_SPACE,
                                 ...
                                 combo_device_list, false);
  combo_device->setLabelPlace(PUPLACE_LOWER_LEFT);
  combo_device->setLabel("Device");
  ...
  combo_speed = new puaComboBox( LABEL_W + DLG_DEF_SPACE,
                                ...
                                (char**)T_TX_InterfaceSerial::baudRates, false);
  combo_speed->setLabelPlace(PUPLACE_LOWER_LEFT);
  combo_speed->setLabel("Baudrate / Sync");
...
void CGUICtrlDeviceDialog::rebuildDeviceComboList()
{
  ...
  else if ((nItem == T_TX_Interface::eIM_zhenhua) || (nItem == T_TX_Interface::eIM_serpic))
  {
...
const char* CGUICtrlDeviceDialog::getDeviceName()
{
  ...
  if (combo_device_list != NULL)
  {
    name = combo_device_list[combo_device->getCurrentItem()];
  }
  return name;
}
...
bool CGUICtrlDeviceDialog::activateSelectedInterface()
{
  ...
  cfgfile->setAttributeOverwrite("inputMethod.method", InputMethodStrings[nItem]);
