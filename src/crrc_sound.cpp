...
struct tagAudio3D
{
  ...
  float flModelVolume;  ///< volume setting for model sounds (0.0 ... 1.0)
} Audio3D;
...
void T_EngineSound::calculate()
{
  ...
  flModelVolume = Audio3D.flModelVolume;
  ...
  else
  {
    ...
    float fEngineVol = (float)SDL_MIX_MAXVOLUME 
                        - ((float)SDL_MIX_MAXVOLUME * flDist / 1000);
...
void T_GliderSound::calculate()
{
  ...
  float flModelVolume = Audio3D.flModelVolume;
 ...
 *  \param flModelVolume  airplane volume control setting
 */
                   ...
                   float flModelVolume)
{
  ...
  Audio3D.flModelVolume = flModelVolume;
  SDL_UnlockAudio();
}
