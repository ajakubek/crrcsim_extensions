...
void Power::Battery::automagic(SimpleXMLTransfer* xml)
{
  Shaft* s;
  s = new Shaft();
  s->automagic(xml);
  ...
  R_I   = 0; 
  U_off = 1;
  U     = xml->getDouble("battery.shaft.engine.automagic.U");
  C_0   = xml->getDouble("battery.shaft.engine.automagic.I")*xml->getDouble("battery.automagic.T");
  xml->setAttribute("battery.C", doubleToString(C_0/60/60));
  xml->setAttribute("battery.U_0",   doubleToString(U));
  xml->setAttribute("battery.U_off", doubleToString(U_off));
  xml->setAttribute("battery.R_I",   doubleToString(R_I));

  throttle_min = xml->getDouble("battery.throttle_min");
  throttle_old = 0;
  
  SimpleXMLTransfer* g = new SimpleXMLTransfer();
  g->setName("U_0rel");
  g->setContent("\n   1.00;\n   1.00;\n");
  xml->getChild("battery")->addChild(g);
  voltage.push_back(U);
  voltage.push_back(U);
...
void Power::Battery::reset(CRRCMath::Vector3 vInitialVelocity)
{
  C.init(C_0, 0);
  throttle_old = 0;
  nUOffStatus = 0; 
  ...
  for (unsigned int n=0; n<shafts.size(); n++)
    shafts[n]->reset(vInitialVelocity);
}
